...
' same as "test6.bas" but with MP3 streaming
...
const scr_height as integer = 200 '200 480 600 ...
  ...
  mov edi,dword ptr [video_mem]
    ...
    asm_for_i:
      ...
      asm_for_r:
        ...
        asm_found_it:
        'mov ch,cl
        mov byte ptr [edi],cl '*video_ptr=itera (color)
        'shr cx,2
        'mov word ptr [esi],cx 
        asm_next:
        add edi,2             ' video_ptr+=1
        'add esi,2
